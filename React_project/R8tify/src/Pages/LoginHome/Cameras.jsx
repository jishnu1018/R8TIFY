  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const filteredProducts = data.filter((item) => item.category === "Cameras");
                  ...
                  {reviews[product._id]?.length > 0 ? (
                    <>
                      ...
                      {reviews[product._id].length > 1 && !reviewsExpanded[product._id] && (
                        <button
                          onClick={() =>
                            setReviewsExpanded((prev) => ({ ...prev, [product._id]: true }))
                          }
                      ...
                      {reviewsExpanded[product._id] && (
                        <button
                          onClick={() =>
                            setReviewsExpanded((prev) => ({ ...prev, [product._id]: false }))
                          }
                          className="text-blue-500 text-sm mt-2 underline hover:text-blue-700"
                        >
                          Show Less
